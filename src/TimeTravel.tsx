...
export function useQueryWithTimeTravel<TData = any, TVariables = OperationVariables>(query: DocumentNode | TypedDocumentNode<TData, TVariables>, options?: QueryHookOptions<TData, TVariables>) {
  ...
  if (block) {
    ...
    // NB: We assume that the query to edit is always the first operation and the first field.
    const argumentIdx = ((query.definitions[0] as OperationDefinitionNode).selectionSet.selections[0] as FieldNode).arguments?.findIndex(
        argument => argument.name.value === 'block');
    if (!argumentIdx) {
      queryToUse = query;
    }
    else {
      queryToUse = update(query as any, {
        definitions: {
          0: {
            selectionSet: {
              selections: {
